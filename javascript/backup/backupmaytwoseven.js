...
document.addEventListener("DOMContentLoaded", function() {
  ...
  mapSVG.style("display", "block");
	...
	const radioButtons = document.querySelectorAll('.accordion-header-2 input[type="radio"]');
	...
	function preprocess_organizational(d){
		return {
					...
					accordion8: d["municipal financial rating"],
					accordion9: d["% residents satisfied with municipal services (survey)"],
					accordion10: d["# non-profit organizations per 10,000 residents"],
					accordion11: d["# full-service grocery stores per 10,000 residents"],
					accordion12: d["# civic organizations per 10,000 residents"]
				}		
	}
...
function showYearInput(accordionId) {
  document.getElementById("year-input-container-" + accordionId).style.display = "block";
  document.getElementById("year-range-input-container-" + accordionId).style.display = "none";
}

function showYearRangeInput(accordionId) {
  document.getElementById("year-input-container-" + accordionId).style.display = "none";
  document.getElementById("year-range-input-container-" + accordionId).style.display = "block";
}

function applyYear(accordionId) {
  var year = document.getElementById("year-input-" + accordionId).value;
  finaldata = finalPreprocessing(accordionId,[year]);
  cityChanged(finaldata);
}

function applyYearRange(accordionId) {
  var startYear = document.getElementById("start-year-" + accordionId).value;
  var endYear = document.getElementById("end-year-" + accordionId).value;
